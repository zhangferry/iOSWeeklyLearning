...
- 你想在 Swift 中使用某个 Objective-C API 时，采用一些 Swift 的特有类型，比如元组。例如，将 Objective-C 的  `- (void)getRed:(nullable CGFloat *)red green:(nullable CGFloat *)green blue:(nullable CGFloat *)blue alpha:(nullable CGFloat *)alpha;` 方法在 Swift 中变为一个只读计算属性，其类型是一个包含 rgba 四个元素的元组 `var rgba: (red: CGFloat, green: CGFloat, blue: CGFloat, alpha: CGFloat)`。
