...
如果 `消息发送` 阶段未能处理未知消息，那么就会进行一次 `动态方法解析`。我们可以在该阶段通过动态添加方法实现，来处理未知消息。`动态方法解析` 后，会再次进入 `消息发送` 阶段，从 “去 receiverClass 的 cache 中查找 IMP” 这一步开始执行。
...
如果我们在该阶段正确地处理了未知消息，那么再次进入 `消息发送` 阶段肯定能找到 IMP 并调用，否则将进入 `消息转发` 阶段。
...
具体来说，就是给 receiver 发送一条如下消息，注意有实例方法和类方法之分。
...
我们可以重写以上方法来处理未知消息。在 forwardInvocation 方法中，我们可以将未知消息转发给其它对象（代价太大，不如在 Fast 处理），或者改变未知消息的内容再转发给其它对象，甚至可以定义任何逻辑。
...
* 以上几个阶段均有机会处理消息，但处理消息的时间越早，性能就越高。
  - 最好在 `动态方法解析` 阶段就处理完，这样 Runtime 就可以将此方法缓存，这样稍后这个对象再接收到同一消息时就无须再启动 `动态方法解析` 与`消息转发`流程。
