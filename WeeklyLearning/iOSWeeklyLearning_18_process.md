...
iOS包的大小，是每个开发这必须关注的问题，对一个大型的项目，只是代码段就有可能超过100M，算上armv7和arm64架构，就会超过200M。 LinkMap工具能够检查哪个类、哪个三方库占用的空间，通过分析项目的LinkMap文件，得出每个类或者库所占用的空间大小（代码段+数据段），方便开发者快速定位需要优化的类或静态库
